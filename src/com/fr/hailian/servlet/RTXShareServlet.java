...
public class RTXShareServlet extends BaseServlet {
	...
	private void overwriteReport(HttpServletRequest request,
		...
		try {
			//用户名
			String userName= AbstractFSAuthService.getUserShowName(request);
			...
			if(StringUtils.isNotBlank(userName)){
				...
				//生成url地址 发送RTX信息使用
				String sign=SymmetricEncoder.createSign(userName);
				String url=domain+"/RTXSecurityServlet?sign="+sign+"&fr_username="+userName;
